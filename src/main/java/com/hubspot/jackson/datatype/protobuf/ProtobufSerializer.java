...
public abstract class ProtobufSerializer<T extends MessageOrBuilder>
  ...
  protected void writeValue(
    ...
    switch (field.getJavaType()) {
      ...
      case MESSAGE:
        ...
        if (serializer == null) {
          serializer = serializerProvider.findValueSerializer(value.getClass());
  ...
  private static boolean writeEnumsUsingIndex(SerializationContext config) {
    return config.isEnabled(EnumFeature.WRITE_ENUMS_USING_INDEX);
  }

  private static JacksonException unrecognizedType(
    FieldDescriptor field,
    JsonGenerator generator
  ) throws JacksonException {
    String error = format(
